#[cfg(test)]
mod eth_client_test {
    use std::str::FromStr;
    use ethers::types::{Address, Bytes};
    use bundler::ethereum::EthClientHandler;
    use bundler::ethereum::types::EthClient;

    #[tokio::test]
    async fn test_eth_estimate_gas() {
        let private_key_hex = "35640e441bf700a1afcdb33eee4cf795013d23195f450cf5f9d80274617b72ec";
        let private_key = hex::decode(private_key_hex).unwrap();

        let eth_client =
            EthClient::new("0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789", private_key.as_slice()).unwrap();

        let from = Address::from_str("0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789").unwrap();
        let to = Address::from_str("0xfFec17D1920455f6AD90E578269832d5c442D59C").unwrap();
        let call_data = Bytes::from("".as_bytes().to_vec());

        let res = eth_client.estimate_gas(from, to, call_data).await;
        assert!(res.is_ok());
    }
}
